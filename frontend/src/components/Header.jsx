...
export default function Header({
  ...
  return (
    <TooltipProvider delayDuration={150}>
      <header className="glass-panel border-b sticky top-0 z-40 px-3 sm:px-6 py-2 transition-all bg-white/95 backdrop-blur-md">
        <div className="w-full max-w-[1440px] mx-auto flex flex-col xl:flex-row xl:items-center xl:justify-between gap-2.5 xl:gap-3">
          ...
          <div className="flex items-center justify-between shrink-0 pl-1 sm:pl-2 mr-1 sm:mr-3">
            <div
              className="flex items-center gap-2 select-none cursor-pointer group py-0.5"
            ...
            >
              <img
                ...
                className="h-10 sm:h-11 md:h-12 w-auto max-w-[190px] sm:max-w-[220px] md:max-w-[250px] object-contain drop-shadow-xs transition-transform duration-200 group-hover:scale-[1.02]"
              />
            </div>
          ...
          <div className="hidden xl:flex items-center justify-end gap-1.5 sm:gap-2 shrink-0">
            ...
            {currentRole === 'MoSPI Reviewer' && onTriggerSync && (
              <Button
                ...
                disabled={isSyncing}
                className="h-8 px-2.5 rounded-xl border-indigo-200 bg-indigo-50/80 text-indigo-700 hover:bg-indigo-100 hover:text-indigo-800 text-xs font-semibold shadow-2xs gap-1 cursor-pointer"
              >
            ...
            <Select value={currentRole} onValueChange={handleRoleChange}>
              <SelectTrigger className="h-8 min-w-[150px] px-2.5 gap-1.5 rounded-xl border-slate-200/90 bg-white text-xs font-medium shadow-2xs hover:bg-slate-50 transition-colors focus:ring-primary/20 cursor-pointer">
            ...
            {loggedInUser && currentRole !== 'Read-Only Public Tier' && (
              <Button
                ...
                size="sm"
                onClick={onLogout}
                className="h-8 px-2 rounded-xl text-xs font-medium text-slate-500 hover:text-slate-800 hover:bg-slate-100 cursor-pointer"
              >
                Logout ({loggedInUser})
              </Button>
            )}
